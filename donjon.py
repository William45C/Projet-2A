...
class ClassDonjon: ##Classe qui gère les salles du donjon
    def __init__(self):
        self.level = 1 ##Niveau du donjon, augmente à chaque fois que le joueur descend d'un étage
        self.current_room = {"devant": "", "gauche": "", "droite": ""} ##Dictionnaire pour stocker les descriptions des trois directions possibles
        self.devant = [
    "une porte suspecte",
    ...
    def generer_nouvelle_salle(self): ##Permet de générer une nouvelle salle à chaque fois que le joueur change de lieu
        """Generates the layout of the next room without blocking the game."""
        murs = sample(range(0, 3), randint(1, 2)) ##Génère aléatoirement 1 ou 2 murs parmi les trois directions possibles (0, 1, 2)
        
        self.current_room["devant"] = "un mur" if 0 in murs else choice(self.devant) ##Si 0 est dans murs, alors devant est un mur, sinon c'est une description aléatoire de la liste devant
        self.current_room["gauche"] = "un mur" if 1 in murs else choice(self.devant) ##Si 1 est dans murs, alors gauche est un mur, sinon c'est une description aléatoire de la liste devant
        self.current_room["droite"] = "un mur" if 2 in murs else choice(self.devant) ##Si 2 est dans murs, alors droite est un mur, sinon c'est une description aléatoire de la liste devant
        
    def showRoom(self): ##Affiche la description de la salle actuelle
        print(self.current_room)
        
    def tenter_deplacement(self, direction, NextLevel=False, personnage=None): ##Tente de déplacer le joueur dans la direction spécifiée
        """
        ...
        if direction == 'i':
            return "inventory"
        else: ##Si la direction n'est pas 'o' (pour "observation"), on vérifie si le déplacement est possible
            mapping = {'z': 'devant', 'q': 'gauche', 'd': 'droite'} 
            cible = mapping.get(direction)

            if cible and "mur" in self.current_room[cible]: ##Si la direction cible est un mur, le déplacement échoue
                return False
            elif NextLevel == True:
                return True
